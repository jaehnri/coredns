...
func TestSetup(t *testing.T) {
	tests := []struct {
		...
		// positive
		{"quic . 127.0.0.1", false, ".", nil, ""},
		{"quic . 127.0.0.1 {\nexcept coredns.io\n}\n", false, ".", nil, ""},
		{"quic . 127.0.0.1", false, ".", nil, ""},
		{"quic . 127.0.0.1:53", false, ".", nil, ""},
		{"quic . 127.0.0.1:8080", false, ".", nil, ""},
		{"quic . [::1]:53", false, ".", nil, ""},
		{"quic . [2003::1]:53", false, ".", nil, ""},
		{"quic . unix:///var/run/g.sock", false, ".", nil, ""},
		// negative
		{"quic . a27.0.0.1", true, "", nil, "not an IP"},
		{"quic . 127.0.0.1 {\nblaatl\n}\n", true, "", nil, "unknown property"},
		{`quic . ::1
		quic com ::2`, true, "", nil, "plugin"},
		{"quic xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx 127.0.0.1", true, "", nil, "unable to normalize 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'"},
	}
...
nameserver 10.10.255.253`), 0666); err != nil {
	...
	tests := []struct {
		...
		// pass
		{`quic . ` + resolv, false, "", []string{"10.10.255.252:53", "10.10.255.253:53"}},
	}
